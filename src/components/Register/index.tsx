    ...
    if (firstInputPassword === false) {
      ...
      if (inputPassword?.length === 0) {
        ...
        if (regRep.test(inputPassword) === true) {
          ...
          if (
            regEng.test(inputPassword) === true &&
            regNum.test(inputPassword) === false &&
            regExp.test(inputPassword) === false
          ) {
            return (
              <>
                숫자, 영문 대소문자, 특수문자 중 두가지 이상으로 조합해
                주십시오.
              </>
            );
          } else if (
            regEng.test(inputPassword) === false &&
            regNum.test(inputPassword) === true &&
            regExp.test(inputPassword) === true
          ) {
            return (
              <>
                숫자, 영문 대소문자, 특수문자 중 두가지 이상으로 조합해
